...
public class Netflow_record {
	...
	private byte[] src_ip_b = new byte[4];
	private byte[] dst_ip_b = new byte[4];
	private byte[] pkt_b = new byte[8];
	private byte[] bytes_b = new byte[8];
	...
	public boolean setNeflowRecord(byte[] record){
		...
		System.arraycopy(record, 28, src_ip_b, 0, 4);
		System.arraycopy(record, 32, dst_ip_b, 0, 4);
		System.arraycopy(record, 36, pkt_b, 0, 8);
		System.arraycopy(record, 44, bytes_b, 0, 8);
		...
		str_src_ip = CommonData.longTostrIp(Bytes.toLong(BinaryUtils.flipBO(src_ip_b, 4)));
		str_dst_ip = CommonData.longTostrIp(Bytes.toLong(BinaryUtils.flipBO(dst_ip_b, 4)));
		pkt = Bytes.toLong(BinaryUtils.flipBO(pkt_b, 8));
		bytes = Bytes.toLong(BinaryUtils.flipBO(bytes_b, 8));
		return true;
